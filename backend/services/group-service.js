...
async function findUserGroups(userid){
  try{
    let user = await User.findById(userid).populate({ path: "groups", model: "Group" })
...
async function findGroupUsers(groupid){
  try{
    let group = await Group.findById(mongoose.Types.ObjectId(groupid)).populate({path: "people", model: "User"})
    return group.people
  }catch(error){
