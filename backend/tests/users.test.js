...
test("UserSignUpErrorTwo", async () => {
  await expect(
    User.signup("masato", "nandate", "masatonandate", "password")
  ).rejects.toThrow();
});
...
test("UserSignUpErrorOne", async () => {
  await expect(User.login()).rejects.toThrow();
});

test("UserSignUpErrorOne", async () => {
  await expect(User.login("fakeuser", "fakepassword")).rejects.toThrow();
});

test("UserSignUpErrorTwo", async () => {
  await expect(User.login("masatonandate", "fakepassword")).rejects.toThrow();
});
